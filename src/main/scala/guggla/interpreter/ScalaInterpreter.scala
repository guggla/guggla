  ...
  protected def preProcess(name: String, code: String, bindings: Bindings): String = {
    ...
    def bind(arg: (String, AnyRef)) = {
      ...
      val implicits =
        ...
        } yield "    implicit def " + methName + "(x: " + className + "): " + intfName + " = x.asInstanceOf[" + intfName + "]"
    ...
    code + NL +
      ...
      "  class " + className + "Args(bindings: guggla.interpreter.Bindings) { " + NL +
      bindings.map(bind).mkString(NL) + NL +
      "  } " + NL +
      "  object " + className + "Runner {" + NL +
      "    def main(bindings: guggla.interpreter.Bindings," + NL +
